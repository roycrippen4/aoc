(** Strings.
    ...
    A string [s] of length [n] is an indexable and immutable sequence of [n]
...
positions  0   1   2   3   4    n-1    n
  ...
  indices  | 0 | 1 | 2 | 3 | ... | n-1 |
           +---+---+---+---+     +-----+
    v}
    - An {e index} [i] of [s] is an integer in the range \[[0];[n-1]\]. It
      represents the [i]th byte (character) of [s] which can be accessed using
      the constant time string indexing operator [s.[i]].
    - A {e position} [i] of [s] is an integer in the range \[[0];[n]\]. It
      represents either the point at the beginning of the string, or the point
      between two indices, or the point at the end of the string. The [i]th byte
      index is between position [i] and [i+1].

    Two integers [start] and [len] are said to define a {e valid substring} of
    [s] if [len >= 0] and [start], [start+len] are positions of [s].

    {b Unicode text.} Strings being arbitrary sequences of bytes, they can hold
    any kind of textual encoding. However the recommended encoding for storing
    Unicode text in OCaml strings is UTF-8. This is the encoding used by Unicode
    escapes in string literals. For example the string ["\u{1F42B}"] is the
    UTF-8 encoding of the Unicode character U+1F42B.

    {b Past mutability.} Before OCaml 4.02, strings used to be modifiable in
    place like {!Bytes.t} mutable sequences of bytes. OCaml 4 had various
    compiler flags and configuration options to support the transition period
    from mutable to immutable strings. Those options are no longer available,
    and strings are now always immutable.
...
(** [init n f] is a string of length [n] with index [i] holding the character
    [f i] (called in increasing index order).
...
(** {1:concat Concatenating}
    ...
    {b Note.} The {!Stdlib.( ^ )} binary operator concatenates two strings. *)
...
(** [split_on_char sep s] is the list of all (possibly empty) substrings of [s]
    that are delimited by the character [sep]. If [s] is empty, the result is
    the singleton list [[""]].
...
(** [map f s] is the string resulting from applying [f] to all the characters of
    [s] in increasing order.

    @since 4.00 *)

val mapi : (int -> char -> char) -> string -> string
(** [mapi f s] is like {!map} but the index of the character is also passed to
    [f].
...
(** [iteri] is like {!iter}, but the function is also given the corresponding
    character index.
...
(** [index_from_opt s i c] is the index of the first occurrence of [c] in [s]
    after position [i] (if any).
...
(** [rindex_from_opt s i c] is the index of the last occurrence of [c] in [s]
    before position [i+1] (if any).
...
(** [is_valid_utf_16be b] is [true] if and only if [b] contains valid UTF-16BE
    data. *)
...
(** [is_valid_utf_16le b] is [true] if and only if [b] contains valid UTF-16LE
    data. *)
